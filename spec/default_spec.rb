...
describe 'phantomjs2::default' do
  ...
  it 'notifies the execute resource' do
    ...
    expect(resource).to notify('execute[phantomjs-install]').to(:run).immediately
  end

  it 'extracts the binary' do
    expect(chef_run).to_not run_execute('phantomjs-install').with(
      command: 'tar -xvjf fsd.tar.bz2'
    )
  end

  it 'notifies the link' do
    resource = chef_run.execute('phantomjs-install')
  ...
  it 'creates the symlink' do
    expect(chef_run).to_not create_link('phantomjs-link /src/phantomjs-1.0.0-linux-x86_64/bin/phantomjs').with(
